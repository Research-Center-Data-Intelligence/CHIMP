...
def _upload_managed_calibration_data(data):
    ...
    url = TRAINING_SERVER_URL + "/managed_datasets"
    ...
    path_name = re.sub(r'[<>:"/\\|?*]', '', f"calibration_{username}_{timestamp}_{user_id}")
    zip_buffer.name=path_name
    # files = {}
...
def add_as_websocket_handler(socket_io: SocketIO, app):
    global _on_connect, _on_disconnect, _process_image, _process_video, _upload_managed_calibration_data
