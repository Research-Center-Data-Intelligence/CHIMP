...
def _extract_png_frames_from_video(
    video_bytes: bytes, frame_count: int = DEFAULT_FRAME_COUNT
    ...
    with tempfile.NamedTemporaryFile(suffix=".webm", delete=True) as temp_file:
        ...
        try:
            ...
            if total_frames > 0:
                ...
                current_index = 0
                target_positions = {
                    index: position for position, index in enumerate(frame_indices)
                }
...
def _infer_pose_tensor(
    ...
    model_selection = f"model={MODEL_NAME}, stage={MODEL_STAGE}"
    if MODEL_SESSION_ID:
        model_selection += f", session_id={MODEL_SESSION_ID}"
    print(f"[YOLO Frontend] Using {model_selection}")
    ...
    return _decode_yolo_pose_output(
        first_output, conf_threshold=conf_threshold, iou_threshold=iou_threshold
    )


def _prepare_tensor_from_image_bytes(
    image_bytes: bytes, size: int = IMAGE_SIZE
...
def video_to_png_frames():
    ...
    try:
        ...
        extracted_frames = extract_png_frames_from_video(
            video_bytes, frame_count=frame_count
        )
        ...
        return (
            jsonify({"error": "Video frame extraction failed", "details": str(ex)}),
            500,
        )
...
def video_to_managed_dataset():
    ...
    try:
        ...
        if not response.ok:
            ...
            logger.error(
                "Training API upload failed with status %s", response.status_code
            )
            return (
                jsonify(
                    {
        ...
        return (
            jsonify({"error": "Upload to managed dataset failed", "details": str(ex)}),
            500,
        )
